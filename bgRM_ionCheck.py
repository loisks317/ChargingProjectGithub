...
# check to see how algorithm is doing
#
# LKS, September 2015
#
...
#   # now load in HOPE data for this
    ...
    # try:
        ...
        ax2=ax1.twinx()
        ax2.plot(chargingTimes,np.array(chargingLine) , lw=6, c='Silver')
        ax2.plot(chargingTimes,np.array(chargingLine), lw=2, c='Black')
        ...
        plt.savefig('countsCompare_'+date+'_ion_'+str(sat[isat])+'_spectrogram.png')
        plt.close(fig)
        os.chdir('..')
   #  except(OSError):
   #     print 'bad date: ' + date  
#
        ...
        os.chdir(subdir_name)#
        fig.set_size_inches(13,9)
        plt.savefig('fluxCompare_'+date+'_ion_'+str(sat[isat])+'_spectrogram.png')
        plt.close(fig)
        os.chdir('..')
