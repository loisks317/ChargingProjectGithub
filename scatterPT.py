...
# calculate the thermal currents
lowTperp=np.array(lowTperp); lowDensity=np.array(lowDensity)
lowTpar=np.array(lowTpar); highTperp=np.array(highTperp)
highDensity=np.array(highDensity); highTpar=np.array(highTpar)
mTperp=np.array(mTperp); mDensity=np.array(mDensity)
mTpar=np.array(mTpar)
lowThcur=lowDensity*(0.094)*np.sqrt((2*lowTperp+lowTpar)/(3.0*kelvin))
mThcur=mDensity*(0.094)*np.sqrt((2*mTperp+mTpar)/(3.0*kelvin))
highThcur=highDensity*(0.094)*np.sqrt((2*highTperp+highTpar)/(3.0*kelvin))
aList=[ lowIDen,lowTpar, lowTperp, lowTC, lowPressure, lowDensity, lowThcur, lowMLT, lowMLAT, lowL]
bList=[highIDen,highTpar, highTperp,highTC, highPressure, highDensity, highThcur, highMLT, highMLAT, highL]
cList=[mIDen, mTpar, mTperp,mTC, mPressure, mDensity, mThcur,mMLT,mMLAT,mL]
...
xlimsAlow=[1e-3,1e6, 1e6, 1e6, 1e-3, 1e-2,1e-1, 0, -20,1]
xlimsBhigh=[1e1,1e9, 1e9, 1e9, 1e2, 1e2,30,24,10,7 ]
names=['Low Ion Density [cm$^{-3}$]','Tpar [K]', 'Tperp [K]', 'T [K]', 'Pressure [nPa]', 'Density [cm$^{-3}$]','Thermal Current [$\mu$A m$^{-2}$]', 'MLT', 'MLAT [Degrees]', 'L' ]
for i in range(len(aList)):
    for j in range(len(aList)):
        # define bins
           ...
           axHistx.tick_params(axis='x', bottom='off', top='off', labelbottom='off')
           axHisty.tick_params(axis='y', bottom='off', top='off', labelbottom='off', labeltop='off', labelleft='off')
       
           #cb.ax.tick_params(labelsize=30)
           subdir_name='DoubleScatterNew'
           if not os.path.exists(subdir_name):
              os.umask(0) # unmask if necessary
              os.makedirs(subdir_name) 
           os.chdir(subdir_name)#
           fig.set_size_inches(13,9)
           plt.savefig(names[i]+ '_'+names[j]+'_threshold='+str(threshold1)+'_'+str(threshold2)+'_hist.png')
           plt.close(fig)
           os.chdir('..')
